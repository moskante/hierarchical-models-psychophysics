...
# Fit functions
# ============================================================
fit_GLM <- function(iter, simul_data) {
  tryCatch({
...
fit_GNM <- function(iter, simul_data) {
  tryCatch({
...
fit_GLMM <- function(iter, simul_data) {
  tryCatch({
...
fit_BHGLM <- function(iter, simul_data) {
  tryCatch({
    ...
    # SAFE INITIALIZATION: Initialize PSE, SIGMA and ALL scale hyperparameters to prevent numerical underflow[cite: 1]
    init_fn  <- function() list(
      pse       = rep(true_fixeff_mu, datistan$nsubj), 
      sigma     = rep(15, datistan$nsubj),
      PSE       = true_fixeff_mu,
      SIGMA     = 15,
      tau_pse   = 10,
      tau_sigma = 5,
      tau_PSE   = 100,  # Expand the prior standard deviation of population mean
      tau_SIGMA = 50    # Expand the prior standard deviation of population scale
    )
    
    fit <- sampling(stan_bhglm, data = datistan, chains = 3, warmup = 3000, iter = 5000, cores = 3, refresh = 0, init = list(init_fn(), init_fn(), init_fn()))[cite: 1]
...
fit_BHGNM <- function(iter, simul_data) {
  tryCatch({
    datistan <- list(y = simul_data$Longer, n = simul_data$Total, nobs = nrow(simul_data), x = simul_data$X, subject = as.integer(simul_data$Subject), nsubj = nlevels(simul_data$Subject))
    
    # SAFE INITIALIZATION: MU, SIGMA and hierarchical scales also initialized safely here[cite: 1]
    init_fn  <- function() list(
      mu         = rep(true_fixeff_mu, datistan$nsubj), 
      sigma      = rep(15, datistan$nsubj), 
      MU         = true_fixeff_mu,
      SIGMA      = 15,
      gamma      = rep(0.01, datistan$nsubj), 
      lambda_raw = rep(0.01, datistan$nsubj),
      tau_mu     = 10,
      tau_sigma  = 5,
      tau_MU     = 100,
      tau_SIGMA  = 50
...
all_methods  <- if (run_stan) c("GLM", "GNM", "GLMM",  "BH-GNM") else c("GLM", "GNM", "GLMM")
...
while (any(n_done < n_target)) {
  simul_data   <- simulate_dataset()
  iter_counter <- iter_counter + 1L
  for (m in all_methods) {
    if (n_done[m] >= n_target) next
    row <- switch(m,
                  "GLM"    = fit_GLM(  iter_counter, simul_data),
                  "GNM"    = fit_GNM(  iter_counter, simul_data),
                  "GLMM"   = fit_GLMM( iter_counter, simul_data),
                  "BH-GLM" = fit_BHGLM(iter_counter, simul_data),
                  "BH-GNM" = fit_BHGNM(iter_counter, simul_data))
    if (!is.null(row)) {
...
build_summary <- function(df) {
  df %>%
    ...
    summarise(
      ...
      rmse_pse       = sqrt(mean(bias_pse^2, na.rm = TRUE)),
      bias_jnd       = mean(bias_jnd, na.rm = TRUE),
      rmse_jnd       = sqrt(mean(bias_jnd^2, na.rm = TRUE)),
