...
fit_GLM <- function(iter, simul_data, true_PSE, true_JND) {
  tryCatch({
    ...
    # --- FIXED: Reconstruct probabilities manually from estimated PSE and JND ---
    ...
    sse_glm <- simul_data %>%
      ...
      mutate(
        ...
        pred_prob = pnorm(eta), # Standard probit link (no guess/lapse parameters)
...
fit_GNM <- function(iter, simul_data, true_PSE, true_JND) {
  tryCatch({
    ...
    # --- UPDATED: Calculate SSE for non-hierarchical GNM ---
    # Intercept guess/lapse column names dynamically if estimated by process_subject
